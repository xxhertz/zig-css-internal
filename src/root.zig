...
fn unload(hInst: ?*anyopaque) u32 {
    ...
    hooking.global_hooks_states.deinit();
...
fn main_thread(hInst: ?*anyopaque) callconv(.winapi) u32 {
    ...
    hooking.global_hooks_states.init(allocator);
    ...
    hooks.create_move_o = @ptrFromInt(hooking.virtual_hook(g_pClientMode, 21, @intFromPtr(&hooks.hk_create_move)));
